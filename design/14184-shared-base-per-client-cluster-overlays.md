...
One sharp edge in the replacement path is worth naming: `EmptyCopy` reseeds
`LbEpsEqualityHash` from backend identity, so a builder starts with no trace of anything the
row's owner folded in afterwards — and `newFinalBackendEndpoints` folds in the attached-policy
hash precisely because policy changes endpoint *output* without changing endpoint content.
`ReplaceEndpoints` therefore folds the source row's hash back in (`FoldVersion`). Dropping it
would serve one policy state's load assignment under another's; carrying it over-discriminates
instead, costing at worst a missed dedup.
...
- `ASSERT_SHARED_PROTO_IMMUTABILITY` — arms the shared-proto mutation tripwire. Off by
  ...
  sets it on both of its helm install branches. The conformance action takes an
